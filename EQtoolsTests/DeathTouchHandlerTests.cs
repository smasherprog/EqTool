...
namespace EQtoolsTests
{
    ...
    public class DeathTouchHandlerTests : BaseTestClass
    {
        ...
        public void Test1()
        {
            var dteffect = spellWindowViewModel.SpellList.FirstOrDefault(a => a.Id.StartsWith("--DT--"));
            Assert.IsNull(dteffect);
            logParser.Push("Dread says 'TINIALITA'", DateTime.Now);
            dteffect = spellWindowViewModel.SpellList.FirstOrDefault(a => a.Id.StartsWith("--DT--"));
            Assert.IsNotNull(dteffect);
            Assert.IsTrue(dteffect.Id == "--DT-- 'TINIALITA'");
        }
