...
namespace EQToolApis.Services
{
    ...
    {
        private readonly Dictionary<int, string> _imageById = [];
        ...
        private void LoadImages(string webRoot)
        {
            ...
            foreach (var line in File.ReadLines(path))
            {
                ...
                if (parts.Length < 3 || !int.TryParse(parts[0].Trim(), out var id))
                {
                    continue;
                }
                ...
                _imageById[id] = img;
                _ = _imageByName.TryAdd(name, img);
                _ = _imageByNorm.TryAdd(NormalizeName(name), img);
            }
        }
        ...
        private void LoadTooltips(string webRoot)
        {
            ...
            while (pos < content.Length)
            {
                ...
                if (startIdx >= 0)
                {
                    ...
                    {
                        _ = _tooltipByName.TryAdd(name, text.Trim());
                        _ = _tooltipByNorm.TryAdd(NormalizeName(name), text.Trim());
                    }
                }
